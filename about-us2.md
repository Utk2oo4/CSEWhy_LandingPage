...
├──────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
│                    SECTION C — OUR JOURNEY / STORY                          │
│                                                                              │
│  Built for Aspirants, by Aspirants (intro block)                           │
│  Preparation Shouldn't Be Limited by Geography (block)                     │
│  More Than Just Resources (block)                                          │
│  Our Belief (closing block)                                                │
│                                                                              │
└──────────────────────────────────────────────────────────────────────────────┘
```

Sections A and B sit on two visually distinct background tones (A = warm peach, B = lighter/white) to create separation without a hard border. Section C continues on the lighter/white background established in Section B, functioning as the long-form narrative continuation of the page.
...
# 7. Section C — "Our Journey / Story" (Long-Form Narrative)

## 7.1 Purpose

Section C is the long-form storytelling block that follows "Who We Are." Where Section B introduces the community at a glance (stats + photo), Section C goes deeper into *why* CSEWhy exists, told in first person ("we"). It should read like a founder's letter, not marketing copy.

## 7.2 Layout

Single centered column of readable prose, broken into four labeled blocks. Each block pairs a short bold sub-heading with 1–3 paragraphs of body copy.
...
```css
margin-bottom: 56px; /* last block: 0 */
```

Optionally, alternate subtle left-aligned accent (e.g., a thin vertical orange rule to the left of each block on desktop) to visually separate the four blocks without heavy dividers — keep this restrained and consistent with the site's minimal-divider language.

## 7.3 Sub-heading style
...
## 7.5 Block 1 — Intro / Origin

Optional small eyebrow above the block (reuse eyebrow style from §6.2):
...
```text
CSEWhy started with a simple question: Why should quality UPSC
preparation depend on where you live, how much you can spend, or
how many resources you can find?

What began as a Twitter page sharing useful information and creating
a voice for aspirants gradually grew into something much bigger. As
we listened to students, understood their struggles, and experienced
the preparation journey ourselves, one thing became clear: aspirants
did not need more noise. They needed better tools, reliable resources,
and a place that understood how they actually prepare.

That idea became CSEWhy.
```

## 7.6 Block 2 — Geography / Accessibility

Sub-heading:

```text
Preparation Shouldn't Be Limited by Geography
```

Body:

```text
UPSC preparation isn't the privilege of a particular city.

You shouldn't have to leave your home, move to Delhi, or spend years
navigating countless sources just to access quality preparation.
Whether you're preparing from a metro city, a small town, or your
own home, you deserve access to the right resources and the right
support.

CSEWhy is built around that belief.

We bring together PYQs, practice, current affairs, courses, study
resources, and more in one place, helping aspirants spend less time
searching and more time actually preparing.
```

Treat the standalone lines ("UPSC preparation isn't the privilege of a particular city." and "CSEWhy is built around that belief.") as short emphasis paragraphs — give them their own line break/spacing rather than merging into a dense block, matching the source copy's rhythm.

## 7.7 Block 3 — Beyond Resources
...
```text
Preparation is not only about collecting material.

It is about knowing what to study, practising consistently,
understanding your mistakes, staying updated, and improving over
time.

That's why CSEWhy is building an ecosystem designed around the
actual preparation journey of an aspirant — from discovering what to
study to practising it, revising it, and staying connected with what
matters.
```

## 7.8 Block 4 — Our Belief (closing / emphasis block)

This is the emotional close of the page and should carry slightly more visual weight than blocks 1–3 — consider a subtly different background treatment (e.g., a soft peach or off-white panel with rounded corners) to set it apart as the page's closing statement.
...
```text
We believe good preparation should be accessible, practical, and
aspirant-centric.

CSEWhy is not built from a distance, trying to tell aspirants what
their problems are. It is built by people who have experienced those
problems themselves.

Real problems. Real experiences. Real solutions.

CSEWhy is our attempt to make UPSC preparation a little simpler, more
accessible, and more meaningful for every aspirant, wherever they are.
```

Give the short triplet line — "Real problems. Real experiences. Real solutions." — distinct visual treatment (e.g., bold, slightly larger, or in `var(--brand-orange)`) as it functions as the section's pull-quote / thesis statement.
...
## 7.9 Responsive behavior for Section C

- Reduce max-width padding on tablet/mobile (`24px` gutters minimum).
- Sub-headings step down to `20px` on mobile.
- Keep paragraph line-length readable — do not let text stretch edge-to-edge even on wide mobile viewports; retain a comfortable `~65–75` character measure.
- The Block 4 panel (if using a background treatment) should retain internal padding so text never touches the panel edge on small screens.
...
Section C sub-headings:             24px / 700 / line-height 1.3
Section C body copy:                17px / 400 / line-height 1.75
Section C pull-quote (Block 4):     19px / 700 / brand orange
...
Section C top/bottom padding:       ~90px
Sub-heading → body copy:            ~16px
Between blocks (1→2→3→4):           ~56px
```
...
- Section C sub-headings step down to `20px`; block spacing reduces to `~40px` between blocks; Block 4's panel padding is preserved so text doesn't touch its edges.
...
- Section C sub-headings ("Built for Aspirants, by Aspirants," etc.) should be real `<h3>` elements (or `<h2>` if Section B's heading is an `<h3>`) so the narrative is navigable via heading structure, not just styled `<div>`/`<p>` text.
...
OurJourney (Section C)
└── JourneyBlock (× 4, data-driven)
    ├── Eyebrow (optional, block 1 only)
    ├── BlockHeading
    ├── BlockBody
    └── PullQuote (optional, block 4 only)
```

`ScriptAnnotation` should be a single reusable component accepting `text`, `rotation`, and `position` props rather than four separate hard-coded elements. `JourneyBlock` should render from an ordered content array (see §13) rather than four hard-coded components, so future edits/additions to the story only touch data.
...
Keep Section C's four narrative blocks data-driven as well, so copy edits and future block additions don't touch markup:

```js
const journeyBlocks = [
  {
    id: "origin",
    eyebrow: "About CSEWhy",
    heading: "Built for Aspirants, by Aspirants",
    paragraphs: [
      "CSEWhy started with a simple question: Why should quality UPSC preparation depend on where you live, how much you can spend, or how many resources you can find?",
      "What began as a Twitter page sharing useful information and creating a voice for aspirants gradually grew into something much bigger. As we listened to students, understood their struggles, and experienced the preparation journey ourselves, one thing became clear: aspirants did not need more noise. They needed better tools, reliable resources, and a place that understood how they actually prepare.",
      "That idea became CSEWhy.",
    ],
  },
  {
    id: "geography",
    heading: "Preparation Shouldn't Be Limited by Geography",
    paragraphs: [
      "UPSC preparation isn't the privilege of a particular city.",
      "You shouldn't have to leave your home, move to Delhi, or spend years navigating countless sources just to access quality preparation. Whether you're preparing from a metro city, a small town, or your own home, you deserve access to the right resources and the right support.",
      "CSEWhy is built around that belief.",
      "We bring together PYQs, practice, current affairs, courses, study resources, and more in one place, helping aspirants spend less time searching and more time actually preparing.",
    ],
  },
  {
    id: "beyond-resources",
    heading: "More Than Just Resources",
    paragraphs: [
      "Preparation is not only about collecting material.",
      "It is about knowing what to study, practising consistently, understanding your mistakes, staying updated, and improving over time.",
      "That's why CSEWhy is building an ecosystem designed around the actual preparation journey of an aspirant — from discovering what to study to practising it, revising it, and staying connected with what matters.",
    ],
  },
  {
    id: "belief",
    heading: "Our Belief",
    paragraphs: [
      "We believe good preparation should be accessible, practical, and aspirant-centric.",
      "CSEWhy is not built from a distance, trying to tell aspirants what their problems are. It is built by people who have experienced those problems themselves.",
    ],
    pullQuote: "Real problems. Real experiences. Real solutions.",
    closingParagraph:
      "CSEWhy is our attempt to make UPSC preparation a little simpler, more accessible, and more meaningful for every aspirant, wherever they are.",
  },
];
```
...
Section C — Block 1 heading:
```text
Built for Aspirants, by Aspirants
```

Section C — Block 1 body:
```text
CSEWhy started with a simple question: Why should quality UPSC preparation depend on where you live, how much you can spend, or how many resources you can find?
...
Section C — Block 2 heading:
```text
Preparation Shouldn't Be Limited by Geography
```

Section C — Block 2 body:
```text
UPSC preparation isn't the privilege of a particular city.
...
Section C — Block 3 heading:
```text
More Than Just Resources
```

Section C — Block 3 body:
```text
Preparation is not only about collecting material.
...
Section C — Block 4 heading:
```text
Our Belief
```

Section C — Block 4 body:
```text
We believe good preparation should be accessible, practical, and aspirant-centric.
...
### Section C
- [ ] All four narrative blocks present in order (Origin, Geography, Beyond Resources, Belief)
- [ ] Sub-headings and body copy match source text exactly
- [ ] Standalone emphasis lines keep their own line breaks rather than being merged into dense paragraphs
- [ ] "Real problems. Real experiences. Real solutions." styled as a distinct pull-quote
- [ ] Block 4 carries slightly more visual weight than Blocks 1–3
...
The About Us page should read as a natural continuation of the CSEWhy homepage's visual system — same orange/peach/charcoal palette, same rounded-card and soft-shadow language, same premium-but-warm tone — while shifting the emphasis from "product/course" messaging toward **community and authenticity**. The handwritten-style annotations and the single real community photograph (rather than stock imagery or generic illustration) are the key emotional differentiators of Sections A and B, while Section C's first-person, founder's-letter tone is what makes the "why" behind CSEWhy feel earned rather than asserted. All three sections together should read as one continuous story, not three disconnected page fragments.
